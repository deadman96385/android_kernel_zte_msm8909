...
#define FLASH_NAME "qcom,led-lm3648"

#define CONFIG_MSMB_CAMERA_DEBUG
#ifdef CONFIG_MSMB_CAMERA_DEBUG
#define LM3648_DBG(fmt, args...) pr_err(fmt, ##args)
#else
#define LM3648_DBG(fmt, args...)
#endif
...
static int32_t msm_lm3648_torch_create_classdev(struct device *dev ,
				void *data)
{
	int rc;
...
int msm_flash_lm3648_led_init(struct msm_led_flash_ctrl_t *fctrl)
{
	uint16_t buf;
	int rc = 0;
	...
	gpio_set_value_cansleep(
		...
		GPIO_OUT_LOW);
	fctrl->flash_i2c_client->i2c_func_tbl->i2c_read(fctrl->flash_i2c_client, 0x0b, &buf, MSM_CAMERA_I2C_BYTE_DATA);
...
int msm_flash_lm3648_led_release(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int msm_flash_lm3648_led_off(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int msm_flash_lm3648_led_low(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int msm_flash_lm3648_led_high(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
		...
		const struct i2c_device_id *id)
{
	...
	if (!rc)
		msm_lm3648_torch_create_classdev(&(client->dev), NULL);
	return rc;
}
...
static int __init msm_flash_lm3648_init(void)
{
	LM3648_DBG("%s entry\n", __func__);
	return i2c_add_driver(&lm3648_i2c_driver);
}

static void __exit msm_flash_lm3648_exit(void)
{
	LM3648_DBG("%s entry\n", __func__);
...
static struct msm_flash_fn_t lm3648_func_tbl = {
	...
	.flash_led_init = msm_flash_lm3648_led_init,
	.flash_led_release = msm_flash_lm3648_led_release,
	.flash_led_off = msm_flash_lm3648_led_off,
...
module_init(msm_flash_lm3648_init);
module_exit(msm_flash_lm3648_exit);
