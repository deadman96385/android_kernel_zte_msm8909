...
#endif
{
	...
	ram.descs = kcalloc(ram.num_descs, sizeof(struct persistent_ram_descriptor),
					GFP_KERNEL);
	...
	pr_info("persistent_ram_init, addr: %lx\n", (long)ram.start);
	pr_info("persistent_ram_init, size: %lx\n", (long)ram.size);
...
static const struct of_device_id msm_ram_console[] = {
