...
static inline ssize_t buffer_size_add_clamp(struct persistent_ram_zone *prz, size_t a)
{
	size_t old;
	size_t new;
...
static void notrace persistent_ram_encode_rs8(struct persistent_ram_zone *prz,
	u8 *data, size_t len, u8 *ecc)
{
	int i;
	u16 par[prz->ecc_size];
										...
										unsigned int start, unsigned int count)
{
	...
	u8 *buffer_end = buffer->data + prz->buffer_size;
	u8 *block;
	u8 *par;
...
static void persistent_ram_update_header_ecc(struct persistent_ram_zone *prz)
{
	...
	persistent_ram_encode_rs8(prz, (u8 *)buffer, sizeof(*buffer),
				  prz->par_header);
}
 ...
 */
	...
	if (offset_in_page(start) != 0) {
		pr_warn("%s: phys address 0x%x not align with page 0x%lx!\n", __func__, start, PAGE_SIZE);
	}
