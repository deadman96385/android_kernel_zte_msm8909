...
#define FTS_VTG_MIN_UV		2850000
#define FTS_VTG_MAX_UV		2850000
...
extern int fts_touch_module_for_lsensor;
#endif
...
static struct work_struct	fts_ts_resume_work;
...
static int fts_read_tp_psensor_data(struct fts_ts_data *data)
{
	...
	pr_info("%s %d, psensor_status %d\n", __func__, __LINE__, psensor_status);
...
static int fts_gpio_configure(struct fts_ts_data *data, bool on)
{
	...
	pr_info("%s:start gpio configure, on=%d\n", __func__, on);
	if (on) {
...
static int fts_ts_start(struct device *dev)
{
	...
	if (data->ts_pinctrl) {
		...
		if (err < 0) {
			...
			pr_info("pzh:Cannot get active pinctrl state\n");
		}
	}
...
err_gpio_configuration:
	if (data->ts_pinctrl) {
		...
		if (err < 0) {
			...
			pr_info("pzh:Cannot get suspend pinctrl state\n");
		}
	}
...
*  Return:
*******************************************************************************/
static void fts_ts_resume(struct work_struct *work)
{
	struct fts_ts_data *data = dev_get_drvdata(dev);
	int err;
...
*  Return:
*******************************************************************************/
static int fts_ts_resume(struct work_struct *work)
{
	return 0;
}
				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_UNBLANK) {
			dev =  &fts_data->client->dev;
			queue_work(fts_ts_resume_wq, &fts_ts_resume_work);
...
#endif
struct fts_ts_data *ft_ts;

static void focaltech_get_vid(
	struct i2c_client *client,
	char *p_vid, unsigned int len, int *p_fw_ver)
{
	char write_buf[] = {0};
	char buf1 = 0, buf2 = 0;
	...
	switch (buf1) {
	case 0x57:
		snprintf(p_vid, len, "Goworld(0x%x)", buf1);
		touch_moudle = GOWORLD;
		break;
	case 0x51:
		snprintf(p_vid, len, "ofilm(0x%x)", buf1);
		touch_moudle = OFILM;
		break;
	case 0x55:
		snprintf(p_vid, len, "laibao(0x%x)", buf1);
		touch_moudle = LAIBAO;
		break;
	case 0x5a:
		snprintf(p_vid, len, "TRULY(0x%x)", buf1);
		touch_moudle = TRULY;
		break;
	case 0x5f:
		snprintf(p_vid, len, "success(0x%x)", buf1);
		touch_moudle = SUCCESS;
		break;
	case 0x60:
		snprintf(p_vid, len, "lead(0x%x)", buf1);
		touch_moudle = LEAD;
		break;
	case 0x5d:
		snprintf(p_vid, len, "BM(0x%x)", buf1);
		touch_moudle = BAOMING;
		break;
	case 0x80:
		snprintf(p_vid, len, "EACH(0x%x)", buf1);
		touch_moudle = EACHOPTO;
		break;
	case 0x87:
		snprintf(p_vid, len, "LCE(0x%x)", buf1);
		touch_moudle = LCE;
		break;
	case 0x8f:
		snprintf(p_vid, len, "CMI(0x%x)", buf1);
		touch_moudle = CMI;
		break;
	default:
		snprintf(p_vid, len, "unknown(0x%x)", buf1);
		touch_moudle = UNKNOWN;
		break;
	}

#if defined(CONFIG_SENSORS_LTR559)
	fts_touch_module_for_lsensor = touch_moudle;
#endif
...

{
	...
	len += snprintf(page + len, size - len, "i2c address : %02x\n", 0x3e);

	focaltech_get_vid(ft_ts->client, (char *)vid, sizeof(vid), &buf);
	if (touch_moudle < FTC_MOUDLE_NUM_MAX)
		ver = focaltech_get_fw_ver(ft_ts->client, ftc_fwfile_table[touch_moudle]);
	len += snprintf(page + len, size - len, "module : %s\n", vid);
	len += snprintf(page + len, size - len, "fw version : %02x\n", buf);
	len += snprintf(page + len, size - len, "update flag : 0x%x\n", ftc_update_flag);


	if (ver !=  -1) {
		len += snprintf(page + len, size - len, "need update : %s\n", (ver > buf ? "yes" : "no"));
		len += snprintf(page + len, size - len, "ready fw version : %02x\n", ver);
	} else {
		len += snprintf(page + len, size - len, "no fw to update\n");
	}
...

{
	...
	if (lock == 1) {
		pr_info("ts already in update return\n");
		return 0;
	}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_dev->name = "Fts-touchscreen";
	...
	fts_ts_resume_wq = create_singlethread_workqueue("fts_ts_resume_wq");
	if (!fts_ts_resume_wq) {
		err = -ESRCH;
		goto exit_create_singlethread;
	}
	INIT_WORK(&fts_ts_resume_work, fts_ts_resume);
	...
	focaltech_get_vid(client, (char *)vid, sizeof(vid), &buf);
...
#ifdef CONFIG_TOUCHSCREEN_FTS_PSENSOR
	pr_info("%s %d, support %d, psensor_support %d\n", __func__, __LINE__,
		fts_psensor_support_enabled(), data->pdata->psensor_support);
	if (fts_psensor_support_enabled() && data->pdata->psensor_support) {
	...
	pr_info("********************Enter CTP Auto Upgrade********************\n");
	fts_ctpm_auto_upgrade_select(client);
#endif
...
free_psensor_pdata:
	...
	if (data->ts_info != NULL)
		devm_kfree(&client->dev, data->ts_info);
irq_free:
