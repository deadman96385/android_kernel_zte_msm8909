...
#if GTP_ICS_SLOT_REPORT
	if (id >= GTP_MAX_TOUCH)
		GTP_INFO("gtp Touch id[%2d] too big!\n", id);
...
#if GTP_COMPATIBLE_MODE
	if ((finger == 0x00) && (ts->chip_type == CHIP_TYPE_GT9F)) {
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR("Group%d is invild Group(Len: %d)!", sensor_id+1, ts->gtp_cfg_len);
		ts->pnl_init_error = 1;
		return -EINVAL;
	}
...
#endif
	{
		...
		if (ret == SUCCESS_OK) {
			GTP_DEBUG("Group%d Version: %d, 0x%02X; IC Version: %d, 0x%02X",
...
s32 gtp_get_ic_version(struct i2c_client *client, u8 *ic_version, int length)
{
	s32 ret = -1;
	...
	if (ic_version) {
		...
		ret = gtp_i2c_read(client, ic_version, length);
		if (ret < 0) {
			GTP_ERROR("GTP read version failed");
			return ret;
		}
...
static int get_gtp_mounle_fw_info(st_fw_head *fw_head)
{
	...
	if (strcmp(gtp_etc_path, "/etc/firmware/") == 0) {
		GTP_INFO("there is no fw file in /etc/firmware\n");
		return -EINVAL;
	}
...

{
	...
	GTP_INFO("%s:---enter---\n", __func__);
	if (*ppos) {
		GTP_INFO("[PARAM]size: %d, *ppos: %d", (int)size, (int)*ppos);
		GTP_INFO("[TOOL_READ]ADB call again, return it.");
		return 0;
	}
	...
	GTP_INFO("etc path:%s\n", gtp_etc_path);
		if (get_gtp_mounle_fw_info(&fw_head) >= 0) {
			...
			if ((((fw_head.vid&0xff)<<8)|((fw_head.vid&0xff00)>>8))
							> ((ic_version[7] << 8) | ic_version[6]))
...
static int goodix_parse_dt(struct device *dev, struct goodix_ts_data *pdata)
{
	...
	pdata->reset_gpio		= of_get_named_gpio(pdata->client->dev.of_node,
		"goodix,reset-gpio", 0);
	GTP_INFO("goodix get reset gpio:%d, irq gpio:%d", pdata->reset_gpio, pdata->irq_gpio);
	return 0;
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_INFO("zte_tzb goodix probe start\n");
	...
	if (refresh == NULL)
		GTP_INFO("proc_create ts_information failed!\n");
	refresh = proc_create("ts_update", 0444, dir, &update_proc_ops);
	if (refresh == NULL)
		GTP_INFO("proc_create ts_information failed!\n");

	GTP_INFO("zte_tzb goodix probe end\n");
