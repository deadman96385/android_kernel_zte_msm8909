...
#endif
extern struct goodix_ts_data *gtp_ts;
char *gtp_file_name;
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR("Invild sensor id(%d) !!!", sensor_id);
		return -ENODEV;
	}
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	GTP_GPIO_OUTPUT(gtp_ts->reset_gpio, 0);
	...
	while (retry++ < 200) {
		...
		if (rd_buf[GTP_ADDR_LENGTH] == 0x0C) {
			GTP_DEBUG("Hold ss51 & dsp confirm SUCCESS_OK");
			break;
		}
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	while (i++ < 5) {
		...
		if (ret > 0) {
			GTP_INFO("[update_cfg]Send config SUCCESS_OK.");
			break;
		}
...
s32 gup_update_proc(void *dir)
{
	...
	while (retry++ < 5) {
		...
		show_len = 90;
		GTP_INFO("[update_proc]UPDATE SUCCESS_OK.");
		retry = 0;
		break;
	}
...
s32 gup_hold_ss51_dsp(struct i2c_client *client)
{
	...
	while (retry++ < 200) {
		...
		if (rd_buf[GTP_ADDR_LENGTH] == 0x0C) {
			GTP_DEBUG("[enter_update_mode]Hold ss51 & dsp confirm SUCCESS_OK");
			break;
		}
		...
		for (i = 0; i < cmp_len; ++i) {
			if (chk_cmp_buf[i] != gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i]) {
				...
				i2c_write_bytes(client, cmp_addr+i,
					&gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i], cmp_len-i);
				GTP_ERROR("Check failed index: %d(%d != %d), redownload chuck",
				i, chk_cmp_buf[i], gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i]);
				break;
			}
		}
		if (chk_fail == 1) {
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
	...
	if (update_msg.fw_total_len != sizeof(gtp_default_FW_fl)) {
		GTP_ERROR("Inconsistent fw size. default size: %d(%dK), file size: %d(%dK)",
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
	...
	while (retry++ < 5) {
		...
		GTP_INFO("[fw_download_proc]UPDATE SUCCESS_OK.");
		break;
	}
