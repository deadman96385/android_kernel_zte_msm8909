...
int gt1x_check_firmware(void)
{
	...
	GTP_INFO("Firmware patch: %02X%02X%02X",
		update_info.firmware->version[0], update_info.firmware->version[1], update_info.firmware->version[2]);
...
u8 *gt1x_get_fw_data(u32 offset, int length)
{
	...
	if (update_info.update_type == UPDATE_TYPE_FILE) {
		...
		ret = update_info.fw_file->f_op->read(update_info.fw_file,
			(char *)update_info.buffer, length, &update_info.fw_file->f_pos);
		if (ret < 0) {
...
int gt1x_load_patch(u8 *patch, u32 patch_size, int offset, int bank_size)
{
	...
	GTP_INFO("Load patch code(size: %d, checksum: 0x%04X, position: 0x%04X, bank-size: %d",
		patch_size, gt1x_calc_checksum(patch, patch_size), 0xC000 + offset, bank_size);
