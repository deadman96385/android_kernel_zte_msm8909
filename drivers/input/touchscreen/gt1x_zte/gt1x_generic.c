...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	ptr += snprintf(ptr, PAGE_SIZE, " == == GT1X default config setting in driver == ==\n");

	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		ptr += snprintf(ptr, PAGE_SIZE, "0x%02X, ", gt1x_config[i]);
		if (i % 10 == 9)
			ptr += snprintf(ptr, PAGE_SIZE, "\n");
	}

	ptr += snprintf(ptr, PAGE_SIZE, "\n");
	ptr += snprintf(ptr, PAGE_SIZE, " == == GT1X config read from chip == ==\n");
	i = gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);

	GTP_INFO("I2C TRANSFER: %d", i);
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		ptr += snprintf(ptr, PAGE_SIZE, "0x%02X, ", temp_data[i]);
		if (i % 10 == 9)
			ptr += snprintf(ptr, PAGE_SIZE, "\n");
	}
	...
	ptr += snprintf(ptr, PAGE_SIZE, "\n");
	/* Touch PID & VID */
	ptr += snprintf(ptr, PAGE_SIZE, " == == GT1X Version Info == ==\n");

	gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
	ptr += snprintf(ptr, PAGE_SIZE, "ProductID: GT%c%c%c%c\n",
		temp_data[0], temp_data[1], temp_data[2], temp_data[3]);
	ptr += snprintf(ptr, PAGE_SIZE, "PatchID: %02X%02X\n", temp_data[4], temp_data[5]);
	ptr += snprintf(ptr, PAGE_SIZE, "MaskID: %02X%02X\n", temp_data[7], temp_data[8]);
	ptr += snprintf(ptr, PAGE_SIZE, "SensorID: %02X\n", temp_data[10] & 0x0F);
...
s32 _do_i2c_read(struct i2c_msg *msgs, u16 addr, u8 *buffer, s32 len)
{
	...
	data = kmalloc(IIC_MAX_TRANSFER_SIZE < (len + GTP_ADDR_LENGTH) ?
		IIC_MAX_TRANSFER_SIZE : (len + GTP_ADDR_LENGTH), GFP_KERNEL);
	if (data == NULL)
		return ERROR_MEM;
...
s32 gt1x_init_panel(void)
{
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH || cfg_len > GTP_CONFIG_MAX_LENGTH) {
		GTP_ERROR("Config group%d is INVALID! You need to check you header file CFG_GROUP section!",
			sensor_id + 1);
		return -EINVAL;
	}
		...
		if (!ret) {
			...
			if (checksum == 0 &&	/* first 3 bytes must be number or char */
				IS_NUM_OR_CHAR(buf[0]) && IS_NUM_OR_CHAR(buf[1]) &&
					IS_NUM_OR_CHAR(buf[2]) && buf[10] != 0xFF) {	/*sensor id == 0xFF, retry */
				break;
			}
...
/*********************below added by SZQ 2016-03-02********************/
	...
	if ((touch_num == 0) && (pre_touch == 0))
		key_value = data[1 + 8 * touch_num];
	else {
		#if (defined(CONFIG_BOARD_BENZ) || defined(CONFIG_BOARD_CAPTAIN) || defined(CONFIG_BOARD_XRAY50))
		if (touch_num == 1) {
			if ((data[3 + 3] | (data[3 + 4] << 8)) >= 795)
	...
	if (key_value || pre_key) {
		...
		case 1:
			...
			input_y = 1380;
			input_w = 255;
			gt1x_touch_down(input_x, input_y, input_w, id);
			break;
		case 2:
			input_x = 357;
			input_y = 1380;
		...
		case 4:
			...
			gt1x_touch_down(input_x, input_y, input_w, id);
			break;
#elif (defined(CONFIG_BOARD_SWEET) || defined(CONFIG_BOARD_SAPPHIRE) || defined(CONFIG_BOARD_ELDEN))
		case 1:
			input_x = 85;
			input_y = 921;
			input_w = 255;
...
/*ergate*/
				...
				/*ergate*/
#ifdef CONFIG_FORCE_TOUCH_NDT
				/*skip when NDT pressure is not available*/
				if (input_w > 0)
#endif
					gt1x_touch_down(input_x, input_y, input_w, i);
...
static void gt1x_init_charger(void)
{
	...
	if (gt1x_parse_chr_cfg(gt1x_version.sensor_id) < 0)
		GTP_ERROR("Error occurred when parse charger config.");
}
...
void gt1x_charger_config(s32 dir_update)
{
	...
	if (gt1x_sc_dev && gt1x_sc_dev->enabled
		...
		return;
#endif
	if (gt1x_get_charger_status() == POWER_SUPPLY_STATUS_CHARGING) {
