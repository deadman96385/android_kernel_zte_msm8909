...
enum DOZE_T gesture_doze_status = DOZE_DISABLED; /* doze status */
...
static s32 hotknot_block_rw(u8 rqst_hotknot_state, s32 wait_hotknot_timeout)
{
	...
	if (wait_hotknot_timeout <= 0) {
		wait_event_interruptible(bp_waiter, force_wake_flag || rqst_hotknot_state ==
			(got_hotknot_state & rqst_hotknot_state));
	else
		wait_event_interruptible_timeout(bp_waiter, force_wake_flag || rqst_hotknot_state ==
			(got_hotknot_state & rqst_hotknot_state), wait_hotknot_timeout);
...
s32 hotknot_event_handler(u8 *data)
{
	...
	if (hotknot_paired_flag) {
		...
		GTP_DEBUG("[0x8800~0x8803] = 0x%x, 0x%x, 0x%x, 0x%x",
			hn_state_buf[0], hn_state_buf[1], hn_state_buf[2], hn_state_buf[3]);
...
static s32 io_iic_read(u8 *data, void __user *arg)
{
	...
	if (!err) {
		...
		if (err) {
			GTP_ERROR("ERROR when copy to user.[addr: %04x], [read length:%d]",
				addr, data_length);
			return ERROR_MEM;
		}
...
s32 gt1x_init_node(void)
{
	...
	memset((u8 *)&gesture_data, 0, sizeof(struct st_gesture_data));
