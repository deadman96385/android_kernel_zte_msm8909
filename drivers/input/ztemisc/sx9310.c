 ...
 */
static ssize_t sx9310_set_threshold_show(struct device *dev,
					 struct device_attribute *attr, char *buf)
{
	u8 reg_value = 0;
	psx93XX_t chip = dev_get_drvdata(dev);
	...
	SX9310_DBG("%s enter\n", __func__);
	read_register(chip, SX9310_CPS_CTRL6_REG, &reg_value);
	SX9310_DBG("%s return %d\n", __func__, reg_value);
	return snprintf(buf, 64, "%d\n", reg_value);
}

/*! \brief sysfs store function for setting threshold
 */
static ssize_t sx9310_set_threshold_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	psx93XX_t chip = dev_get_drvdata(dev);
	...
	if (kstrtoul(buf, 0, &val))
		return -EINVAL;
	SX9310_DBG("%s enter(val=%ld)\n", __func__, val);

	if (val >= 0 && val < 32) {
		SX9310_DBG("%s set threshold\n", __func__);
		write_register(chip, SX9310_CPS_CTRL6_REG, val);
	} else {
		SX9310_DBG("%s wrong threshold value!!!\n", __func__);
	}
	return count;
}

static DEVICE_ATTR(set_threshold, 0664, sx9310_set_threshold_show, sx9310_set_threshold_store);
...
int sx93XX_init(psx93XX_t chip)
{
	...
	if (chip) {
		...
		err = initialize(chip);
		if (err == 0) {
			dev_err(chip->pdev, "Succed to initialize\n");
			return 0;
		}
