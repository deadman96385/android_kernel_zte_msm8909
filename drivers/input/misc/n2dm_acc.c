...
#define N2DM_ACC_ODR0_PWD_DOWN		(0x00)	/* xym add power down mode*/
...
#define N2DM_ACC_ODR1620		(0x80)	/* 1620Hz output data rate */
...
{
	int rc = 0;
	static struct regulator *vdd_hold = NULL;
	...
	if (on) {
		...
		if (regulator_count_voltages(vdd_hold) >  0) {
			rc = regulator_set_voltage(vdd_hold,	n2dm_acc_vreg[0].min_uV,
										n2dm_acc_vreg[0].max_uV);
			if (rc) {
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"i2c_smbus_read_i2c_block_data: ret:%d len:%d, command=0x%02x,",
				ret,	len,	cmd);
			unsigned int ii;

			for (ii = 0; ii < len; ii++)
				dev_warn(&stat->client->dev, "buf[%d]=0x%02x, ", ii, buf[ii]);
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"i2c_smbus_write_i2c_block_data: ret=%d, len:%d, command=0x%02x,",
				ret,	len,	reg);
			unsigned int ii;

			for (ii = 0; ii < (len + 1); ii++)
				dev_warn(&stat->client->dev, "value[%d]=0x%02x, ", ii, buf[ii]);
...
static int n2dm_acc_hw_init(struct n2dm_acc_status *stat)
{
	...
	pr_info("%s: cgh please notify %d\n",	__func__,	stat->client->addr);
	buf[0] = WHO_AM_I;
	err = n2dm_acc_i2c_read(stat,	buf,	1);
	if (err < 0) {
		stat->client->addr = 0x9;
		pr_info("%s: cgh1 please notify %d\n",	__func__,	stat->client->addr);
		err = n2dm_acc_i2c_read(stat,	buf,	1);
		if (err < 0) {
	...
	pr_info("%s: write 0X%X to CTRL_REG1\n",	__func__,	buf[1]);
							...
							int poll_interval_ms)
{
	...
	{
		config[0] = CTRL_REG1;
		pr_info("%s: write %X to CTRL_REG1\n",	__func__,	config[1]);
		...
		switch (data) {
		case N2DM_ACC_ODR0_PWD_DOWN:
			dev_info(&stat->client->dev,	"ODR = 0 Hz,	in POWERDOWN MODE !!!\n");
		case N2DM_ACC_ODR1:
					 ...
					 const char *buf,	size_t size)
{
	...
	if (kstrtou64(buf, 10, &interval_ms))
		return -EINVAL;
	if (!interval_ms)
		return -EINVAL;
	dev_info(&stat->client->dev,	"%s: %llu ms\n", __func__, interval_ms);
					...
					const char *buf,	size_t size)
{
	...
	case 16:
		...
		break;
	default:
		dev_err(&stat->client->dev, "invalid range request: %llu, discarded\n", val);
		return -EINVAL;
	}
	mutex_lock(&stat->lock);
	...
	mutex_unlock(&stat->lock);
	dev_info(&stat->client->dev,	"range set to: %llu g\n",	val);
