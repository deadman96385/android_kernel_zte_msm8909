...
static struct sensors_classdev sensors_cdev_9916 = {
	.name = "akm09916-mag",
	.vendor = "Asahi Kasei Microdevices Corporation",
	.version = 1,
	.handle = SENSORS_MAGNETIC_FIELD_HANDLE,
	...
	struct akm_compass_data *akm,	char const *buf,	size_t count,	int pos)
{
	...
	if (kstrtos64(buf,	AKM_BASE_NUM, &val))
		return -EINVAL;
	...
	struct i2c_client *client)
{
	...
	if ((akm->sense_info[0] != AK09911_WIA1_VALUE) ||
			((akm->sense_info[1] != AK09911_WIA2_VALUE) && (akm->sense_info[1] != AK09916_WIA2_VALUE))) {
		dev_err(&client->dev,
...
int akm_compass_probe(struct i2c_client *client,	const struct i2c_device_id *id)
{
	...
	if (s_akm->sense_info[1] == AK09916_WIA2_VALUE)
		s_akm->cdev = sensors_cdev_9916;
	else
