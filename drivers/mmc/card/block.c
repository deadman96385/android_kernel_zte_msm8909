...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (cmd_flags & REQ_DISCARD) {
		...
		if (card->quirks & MMC_QUIRK_SEC_ERASE_TRIM_REQUIRE || (cmd_flags & REQ_SECURE &&
...
static const struct mmc_fixup blk_fixups[] =
{
	...
	MMC_FIXUP("SD128", CID_MANFID_KINGSTON2, CID_OEMID_ANY, add_quirk_sd,
		  MMC_QUIRK_SEC_ERASE_TRIM_REQUIRE),
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if (mmc_card_mmc(card)) {
		...
		if (strnstr(saved_command_line, RECOVERY_MODE_STR, sizeof(saved_command_line))) {
			...
			/*sbl1+DDR+rpm+tz+fsg+sec+devinfo+echarge+splash+aboot+modem+boot+recovery+pad+pad1+ztelk */
			set_partition_ro(card, md, "sbl1", 16);
