...
static int max17055_set_temp(struct max17055_chip *chip)
{
	int ret = 0;
	struct regmap *map = chip->regmap;
	...
	pr_debug("battery temp is :%d\nj", prop.intval);
	regmap_update_bits(map, MAX17055_Config, CONFIG_TEX_BIT, CONFIG_TEX_BIT);
	ret = regmap_write(map, MAX17055_Temp, prop.intval * 256 / 10);
	return ret;
}
...
static void max17055_config_simple(struct max17055_chip *chip, bool with_ini)
{
	...
	if (chip->chip_type == MAXIM_DEVICE_TYPE_MAX17055A)
		max17055_write_volatile_reg(map, MAX17055_RepCap, 0);
